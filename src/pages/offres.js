...
import { OffresTable } from 'src/sections/offres/offres-table';
import { OffresSearch } from 'src/sections/offres/offres-search';
...
const useOffreIds = (offres) => {
  return useMemo(
    () => {
      return offres.map((customer) => customer.id);
    },
    [offres]
  );
};
...
const Page = () => {
  ...
  const offres = useOffres(page, rowsPerPage);
  const offresIds = useOffreIds(offres);
